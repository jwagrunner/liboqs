...
OQS_KEM *OQS_KEM_rlce_l1_new() {
	...
	kem->length_public_key = RLCE_L1_pub_key_size;
	kem->length_secret_key = RLCE_L1_sec_size;
	kem->length_ciphertext = RLCE_L1_cipherLen;
	kem->length_shared_secret = OQS_KEM_RLCE_ss_len;
	
	kem->keypair = crypto_kem_keygenerate1;
  kem->encaps = crypto_kem_encapsulate1;
	kem->decaps = crypto_kem_decapsulate1;
	
	return kem;
}
OQS_API OQS_STATUS crypto_kem_keygenerate1(unsigned char *pk, unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_keygenerateX(0,pk,sk); 
}
OQS_API OQS_STATUS crypto_kem_encapsulate1(unsigned char *ct,unsigned char *ss,const unsigned char *pk) {
  return (OQS_STATUS) crypto_kem_encapsulateX(RLCE_L1_pub_key_size, RLCE_L1_cipherLen,ct,ss,pk);
}
OQS_API OQS_STATUS crypto_kem_decapsulate1(unsigned char *ss,const unsigned char *ct,const unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_decapsulateX(RLCE_L1_sec_size, RLCE_L1_cipherLen,ss,ct,sk);
}
#endif
 ...
 OQS_KEM *OQS_KEM_rlce_l3_new() {
	...
	kem->length_public_key = RLCE_L3_pub_key_size;
	kem->length_secret_key = RLCE_L3_seckey_size;
	kem->length_ciphertext = RLCE_l3_cipherLen;
	kem->length_shared_secret = OQS_KEM_RLCE_ss_len;
	
	kem->keypair = crypto_kem_keygenerate3;
  kem->encaps = crypto_kem_encapsulate3;
	kem->decaps = crypto_kem_decapsulate3;
	
	return kem;
}
OQS_API OQS_STATUS crypto_kem_keygenerate3(unsigned char *pk, unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_keygenerateX(1,pk,sk); 
}
OQS_API OQS_STATUS crypto_kem_encapsulate3(unsigned char *ct,unsigned char *ss,const unsigned char *pk) {
  return (OQS_STATUS) crypto_kem_encapsulateX(RLCE_L3_pub_key_size, RLCE_l3_cipherLen,ct,ss,pk);
}
OQS_API OQS_STATUS crypto_kem_decapsulate3(unsigned char *ss,const unsigned char *ct,const unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_decapsulateX(RLCE_L3_seckey_size, RLCE_l3_cipherLen,ss,ct,sk);
}
#endif
 ...
 OQS_KEM *OQS_KEM_rlce_l5_new() {
	...
	kem->length_public_key = RLCE_L5_pub_key_size;
	kem->length_secret_key = RLCE_L5_seckey_size;
	kem->length_ciphertext = RLCE_l5_cipherLen;
	kem->length_shared_secret = OQS_KEM_RLCE_ss_len;
	
	kem->keypair = crypto_kem_keygenerate5;
  kem->encaps = crypto_kem_encapsulate5;
	kem->decaps = crypto_kem_decapsulate5;
	
	return kem;
}
OQS_API OQS_STATUS crypto_kem_keygenerate5(unsigned char *pk, unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_keygenerateX(2,pk,sk); 
}
OQS_API OQS_STATUS crypto_kem_encapsulate5(unsigned char *ct,unsigned char *ss,const unsigned char *pk) {
  return (OQS_STATUS) crypto_kem_encapsulateX(RLCE_L5_pub_key_size, RLCE_l5_cipherLen,ct, ss,pk);
}
OQS_API OQS_STATUS crypto_kem_decapsulate5(unsigned char *ss,const unsigned char *ct,const unsigned char *sk) {
  return (OQS_STATUS) crypto_kem_decapsulateX(RLCE_L5_seckey_size, RLCE_l5_cipherLen,ss,ct,sk);
}
#endif
...
OQS_API OQS_STATUS crypto_kem_decapsulateX(size_t secLen, size_t cipherLen, unsigned char *ss,const unsigned char *ct,const unsigned char *sk) {
  int ret;
  RLCE_private_key_t RLCEsk=B2sk(sk, secLen);
  if (RLCEsk==NULL) return (OQS_STATUS) -1;
  unsigned char message[RLCEsk->para[6]];
  unsigned long long mlen=RLCEsk->para[6];
  ret=RLCE_decrypt((unsigned char *)ct,cipherLen,RLCEsk,message,&mlen);
  if (ret<0) return (OQS_STATUS) ret;
  memcpy(ss, message, OQS_KEM_RLCE_ss_len);
  return (OQS_STATUS) ret;
}
...
RLCE_private_key_t RLCE_private_key_init (size_t para[]) {
  ...
  key->S = matrix_init(para[1], para[15]+1);
...
int RLCE_key_setup (unsigned char entropy[], int entropylen,
  ...
  int seed_material_len=entropylen+noncelen+perlen+addlen;
  unsigned char seed_material[seed_material_len];
  memcpy(seed_material,entropy, entropylen);
  memcpy(&(seed_material[entropylen]),nonce, noncelen);
  memcpy(&(seed_material[entropylen+noncelen]),pers, perlen);
  memcpy(&(seed_material[entropylen+noncelen+perlen]),addS, addlen);
	int rc = OQS_randombytes_switch_algorithm(OQS_RAND_alg_nist_kat);
	if (rc != OQS_SUCCESS) return DRBGFAIL;
	OQS_randombytes_nist_kat_init_256bit(seed_material, &(seed_material[48]));
	OQS_randombytes(randomBytes, nRB);
...
int RLCE_encrypt(unsigned char msg[], unsigned char entropy[], size_t entropylen,
  ...
  int seed_material_len=entropylen+noncelen+perslen+addlen;
  unsigned char seed_material[seed_material_len];
  memcpy(seed_material,entropy, entropylen);
  memcpy(&(seed_material[entropylen]),noncenew, noncelen);
  memcpy(&(seed_material[entropylen+noncelen]),pers, perslen);
  memcpy(&(seed_material[entropylen+noncelen+perslen]),add, addlen);
	int rc = OQS_randombytes_switch_algorithm(OQS_RAND_alg_nist_kat);
	if (rc != OQS_SUCCESS) return DRBGFAIL;
	OQS_randombytes_nist_kat_init_256bit(seed_material, &(seed_material[48]));
...
int getrandombytesfromcommandline(unsigned char* randomness, int numR) {
  ...
  OQS_SHA2_sha512((uint8_t*) md, (uint8_t*) str,  2*numR);
